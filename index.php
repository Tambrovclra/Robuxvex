...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}
    ...
    /* ===== REMOVE BLUE OUTLINE (theo yêu cầu) ===== */
...
.card-head .title::before {
  ...
  border-radius:3px;
  display:block;
  background: linear-gradient(180deg, #ef4444 0%, #7c3aed 100%);
}
.card-head .meta{
  color:#6b7280;
  font-weight:600;
...
thead th{
  ...
  font-weight:700;
  font-size:13px;
  border-bottom:3px solid #fff;   /* gạch trắng ngang */
  border-right:1px solid #fff;    /* gạch trắng dọc */
}
thead th:last-child{
  border-right:none;
}
...
.chip{
  ...
  font-size:13px;
  color:#fff;
  background: linear-gradient(90deg, #f7c600 0%, #2ecc71 100%);
...
.nav-balance {
  position: static;       /* bỏ absolute đi */
  margin-right: 8px;      /* cách icon tài khoản một chút */
  font-size: 13px;
  font-weight: 700;
  color: #CC0000;         /* vàng cho dễ nhìn, bạn đổi màu tùy ý */
  display: inline-block;
}
  </style>
</head>
...
<header class="navbar">
  ...
  <!-- thêm nav-right để chứa số dư và icon -->
  <div class="nav-right" style="display:flex; align-items:center; gap:8px;">
    <span id="navBalance" class="nav-balance">0đ</span>
    <button class="icon-btn" id="btnAccount" title="Tài khoản">
      <svg class="icon" viewBox="0 0 24 24">
...
<li><a class="nav-link" href="lichsu.html">
  <svg class="nav-ico" viewBox="0 0 24 24">
    <path d="M3 4h18M8 2v4M16 2v4"></path>
    <rect x="3" y="8" width="18" height="12" rx="2"></rect>
    <path d="M7 12h10M7 16h7"></path>
  </svg>
  Lịch sử mua hàng
</a></li>
  ...
  <!-- CONTENT -->
  <main class="wrap">
    <!-- Subtitle / breadcrumb: Trang chủ -->
    <div class="crumb">
      <a href="index.html" id="linkHome">Trang chủ</a>
      <span class="sr"></span>
    </div>
    ...
    <section class="grid-4">
      ...
      <div class="card">
        <div class="kpi">
          <div>
            <h4>Robux có sẵn</h4>
            <div class="val" id="robuxAvailable">852</div>
          </div>
  ...
  <main class="wrap">
    ...
    <!-- TOP MUA ROBUX -->
    <section class="table-card" aria-label="Top mua robux">
      <div class="card-head">
        <div class="title">Top Mua Robux (Web)</div>
        <div class="meta">Realtime</div>
      </div>

      <div class="table-wrap">
        <table role="table" aria-label="Top mua robux table" id="ordersTable">
          <thead>
            <tr>
              <th style="width:45%;">Tài khoản</th>
              <th style="width:30%;">Tổng Robux</th>
    ...
    <!-- TOP NẠP TIỀN -->
    <section class="table-card" aria-label="Top nạp tiền">
      <div class="card-head">
        <div class="title">Top Nạp Tiền</div>
        <div class="meta">Nạp gần đây</div>
      </div>

      <div class="table-wrap">
        <table role="table" aria-label="Top nạp tiền table" id="depositsTable">
          <thead>
            <tr>
              <th style="width:65%;">Tài khoản</th>
              <th style="width:35%;">Số tiền nạp</th>
            </tr>
          </thead>
          <tbody>
            <!-- Firestore sẽ render dữ liệu tại đây -->
  ...
  <script type="module">
    ...
    // ==== Firebase config (giữ như file trước của bạn) ====
    ...
    async function renderAccountBox(){
      ...
      if (!user){
        dropdown.innerHTML = `
          <div class="drop-head">
            <h4 class="who">Bạn chưa đăng nhập</h4>
            <div class="sr">Đăng nhập để theo dõi đơn & số dư</div>
          </div>
          <div class="drop-actions">
            <a class="btn btn-grad-2" href="dk.html">📝 Đăng ký</a>
            <a class="btn btn-grad-1" href="dn.html">🔑 Đăng nhập</a>
          </div>
        `;
        return;
      }
      ...
      const name = data.displayName || user.displayName || (user.email ? user.email.split("@")[0] : "Người dùng");
      ...
      dropdown.innerHTML = `
        <div class="drop-head">
          ...
          <div class="kv">
            <div class="box"><div class="v">${balance}đ</div><div class="sr">Số dư</div></div>
            <div class="box"><div class="v">${deposited}đ</div><div class="sr">Đã nạp</div></div>
            <div class="box"><div class="v">${role}</div><div class="sr">Vai trò</div></div>
          </div>
        </div>
        <div class="drop-actions">
          <a class="btn btn-ghost" href="profile.html">👤 Hồ sơ</a>
          <a class="btn btn-danger" id="btnLogout" href="#">🚪 Đăng xuất</a>
        </div>
      `;
    ...
    async function loadDashboard(){
      ...
      if (!u) {
        // vẫn hiển thị Robux có sẵn = 7152 dù chưa đăng nhập
        document.getElementById('robuxAvailable').textContent = (902).toLocaleString('vi-VN');
        return;
      }
      const qy = query(
      ...
      // Theo yêu cầu: Robux có sẵn = 7152 (cố định)
      document.getElementById("robuxAvailable").textContent = (10.102).toLocaleString('vi-VN');
    }
    ...
    async function loadTopDeposits(){
      ...
      top.forEach(([uid,total])=>{
        tb.insertAdjacentHTML("beforeend", `
          <tr>
            <td class="name">${names[uid] || uid}</td>
            <td><span class="chip">${total.toLocaleString('vi-VN')}đ</span></td>
          </tr>
        `);
      });
    }
  ...
  if (user){ 
    ...
    if (snap.exists()){
      ...
      navBal.textContent = (Number(data.balance||0).toLocaleString("vi-VN")) + "đ";
    } else {
      navBal.textContent = "0đ";
    }
  } else {
    if (navBal) navBal.textContent = "0đ";
    document.getElementById('robuxAvailable').textContent = 
      (7152).toLocaleString('vi-VN');
  }
});
  </script>
  <script type="module">
  ...
  // danh sách link cần kiểm soát
  const protectedLinks = [
    { selector: 'a[href=".html"]', target: ".html" },
    { selector: 'a[href="nap.html"]', target: "nap.html" },
    { selector: 'a[href="lichsu.html"]', target: "lichsu.html" }
  ];

  function protectLinks(user) {
    protectedLinks.forEach(link => {
      ...
      el.addEventListener("click", (e) => {
        ...
        if (user) {
          // đã đăng nhập -> đi tới trang gốc
          window.location.href = link.target;
        } else {
          // chưa đăng nhập -> ép sang đăng ký
